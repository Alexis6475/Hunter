              ...
              <table style={{width:"max-content",minWidth:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>{pg.map(function(c,i){var q=store.qual(c.id);var hf=store.hasFile(c.id);var isH=hovRow===c.id;
                  ...
                  function renderCell(col){
                    ...
                    if(col.k==="revenueBnEur")return<EditCell value={v} onSave={function(x){updateCo(c.id,col.k,x)}} type="number" render={function(x){return<span style={{fontWeight:800,color:Dk,fontVariantNumeric:"tabular-nums"}}>{Number(x).toFixed(2)}</span>}}/>;
