...
function useStore(){const[data,setData]=useState(()=>{try{return JSON.parse(localStorage.getItem("at_v4")||"{}")}catch{return{}}});const save=(id,key,val)=>{setData(p=>{const next={...p,[id]:{...p[id],[key]:val}};try{localStorage.setItem("at_v4",JSON.stringify(next))}catch{}return next})};const get=id=>data[id]||{};const qual=id=>{const d=data[id];if(!d?.scores||!Object.keys(d.scores).length)return null;return getQ(calcWS(d.scores))};const hasFile=id=>{const d=data[id];return d?.prospect&&Object.values(d.prospect).some(v=>v&&v.trim&&v.trim())};return{save,get,qual,hasFile}}
...
/* ═══ CHARTS ═══ */
function useChart(renderFn,deps){const ref=useRef(),cRef=useRef(),[w,setW]=useState(400);useEffect(()=>{const ro=new ResizeObserver(e=>{const ww=e[0]?.contentRect.width;if(ww>50)setW(ww)});if(cRef.current)ro.observe(cRef.current);return()=>ro.disconnect()},[]);useEffect(()=>{if(ref.current)renderFn(ref.current,w)},[w,...deps]);return{cRef,ref,w}}

function BarChart({data,colorMap,height=220}){
  const sorted=[...data].sort((a,b)=>b.v-a.v);const max=Math.max(...sorted.map(d=>d.v),1);
  return<div style={{display:"flex",flexDirection:"column",gap:6}}>{sorted.map(d=><div key={d.k} style={{display:"flex",alignItems:"center",gap:10}}>
    <div style={{width:120,fontSize:10,fontWeight:600,color:"#6b7280",textAlign:"right",flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.k}</div>
    <div style={{flex:1,height:22,background:"#f3f4f6",borderRadius:6,overflow:"hidden",position:"relative"}}>
      <div style={{height:"100%",width:`${(d.v/max)*100}%`,background:colorMap[d.k]||P,borderRadius:6,transition:"width .5s"}}/>
      <span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",fontSize:10,fontWeight:800,color:d.v/max>.25?"#fff":"#374151"}}>{d.v}</span>
    </div>
  </div>)}</div>;
}

function DonutChart({data,colorMap,size=180}){
  const ref=useRef();
  useEffect(()=>{if(!ref.current)return;const svg=d3.select(ref.current);svg.selectAll("*").remove();const r=size/2,g=svg.append("g").attr("transform",`translate(${r},${r})`);const pie=d3.pie().value(d=>d.v).sort(null).padAngle(.02);const arc=d3.arc().innerRadius(r*.55).outerRadius(r-3);g.selectAll("path").data(pie(data)).join("path").attr("d",arc).attr("fill",d=>colorMap[d.data.k]||"#ccc").attr("stroke","#fff").attr("stroke-width",2);g.append("text").attr("text-anchor","middle").attr("dy",".1em").attr("font-size",16).attr("font-weight",900).attr("fill",Dk).text(data.reduce((s,d)=>s+d.v,0).toLocaleString())},[data]);
  return<svg ref={ref} viewBox={`0 0 ${size} ${size}`} style={{width:size,height:size}}/>;
}
...
function DynWaterfall({data}){
  const{cRef,ref,w}=useChart((el,width)=>{
    ...
    let run=0;const bars=steps.map(s=>{let y0,y1;if(["total","sub","acc","fin"].includes(s.t)){y0=0;y1=Math.abs(s.v);run=Math.abs(s.v)}else{y0=run+s.v;y1=run;run=y0}return{...s,y0,y1}});
    for(let i=0;i<bars.length-1;i++){const c=bars[i],n=bars[i+1],fy=["total","sub","acc","fin"].includes(c.t)?y(c.y1):y(c.y0);g.append("line").attr("x1",x(c.l)+x.bandwidth()).attr("x2",x(n.l)).attr("y1",fy).attr("y2",fy).attr("stroke","#d1d5db").attr("stroke-dasharray","3,3")}
    const bg=g.selectAll(".b").data(bars).join("g");
    bg.append("rect").attr("x",d=>x(d.l)).attr("width",x.bandwidth()).attr("y",d=>y(Math.max(d.y0,d.y1))).attr("height",0).attr("rx",5).attr("fill",d=>d.t==="total"?Dk:d.t==="sub"?"#475569":d.t==="acc"?P:d.t==="fin"?"#16a34a":"#e5e7eb").transition().duration(500).delay((_,i)=>i*60).attr("height",d=>Math.abs(y(d.y0)-y(d.y1)));
...
function PrequalPanel({co,store,onClose}){
  ...
  return<Side onClose={onClose}><PHead title={`🎯 ${co.name}`} sub={`${co.segment} · ${co.region} · ${co.revenue}m€`} onSave={()=>{setOk(true);setTimeout(()=>setOk(false),1200)}} saved={ok} onClose={onClose}/>
    <div style={{padding:"16px 20px"}}>
      ...
      {PQ.map(cat=><div key={cat.cat} style={{marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7}}><div style={{width:3,height:14,borderRadius:3,background:cat.color}}/><span style={{fontSize:10,fontWeight:900,textTransform:"uppercase",letterSpacing:".1em",color:cat.color}}>{cat.cat}</span></div>
        {cat.items.map(it=><div key={it.id} style={{...cd,padding:"12px 14px",marginBottom:7,borderLeft:`3px solid ${scores[it.id]?cat.color:"#e5e7eb"}`}}>
          ...
          <button onClick={()=>setSQ(p=>({...p,[it.id]:!p[it.id]}))} style={{marginTop:4,background:"none",border:"none",cursor:"pointer",fontSize:10,color:P,fontFamily:F,padding:0,fontWeight:700}}>{showQ[it.id]?"▾":"▸"} Questions</button>
          {showQ[it.id]&&<div style={{marginTop:3,paddingLeft:8,borderLeft:"2px solid #e5e7eb"}}>{it.q.map((q,i)=><p key={i} style={{margin:"1px 0",fontSize:10,color:"#6b7280"}}>→ {q}</p>)}</div>}
          <textarea placeholder="Notes…" value={notes[it.id]||""} onChange={e=>setN(p=>({...p,[it.id]:e.target.value}))} rows={2} style={{...inp,marginTop:5,fontSize:12}}/>
        </div>)}</div>)}</div></Side>;
}

function ProspectPanel({co,store,onClose}){
  const s0=store.get(co.id);const defs={industry:`${co.segment} — ${co.subSegment}`,companyProfile:`${co.name}\nNIP: ${co.nip}\nRegion: ${co.region}\nRevenue: ${co.revenue}m€\nEmployees: ${co.employees.toLocaleString()}\nSites: ${co.sites}\nScale: ${co.businessScale}`};
  const[fields,setF]=useState({...defs,...(s0.prospect||{})});const[ok,setOk]=useState(false);
  ...
  return<Side onClose={onClose}><PHead title={`📋 ${co.name}`} sub={`${co.segment} · ${co.region} · ${co.revenue}m€`} onSave={()=>{setOk(true);setTimeout(()=>setOk(false),1200)}} saved={ok} onClose={onClose}/>
    <div style={{padding:"16px 20px"}}>
      <div style={{...cd,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:10,fontWeight:700,color:"#6b7280"}}>{filled}/{total}</span><div style={{flex:1,height:5,background:"#e5e7eb",borderRadius:20,overflow:"hidden"}}><div style={{height:"100%",width:`${(filled/total)*100}%`,background:P,borderRadius:20}}/></div></div>
      {PF_SEC.map(sec=><div key={sec.id} style={{marginBottom:16}}><div style={{display:"flex",alignItems:"center",gap:5,marginBottom:7}}><span style={{fontSize:14}}>{sec.icon}</span><span style={{fontSize:12,fontWeight:800,color:Dk}}>{sec.title}</span></div>
        <div style={{...cd,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>{sec.fields.map(f=><div key={f.id}><label style={{display:"block",fontSize:9,fontWeight:800,color:"#9ca3af",textTransform:"uppercase",letterSpacing:".08em",marginBottom:2}}>{f.l}</label>
          {f.t==="area"?<textarea value={fields[f.id]||""} onChange={e=>setF(p=>({...p,[f.id]:e.target.value}))} rows={3} style={inp}/>:<input value={fields[f.id]||""} onChange={e=>setF(p=>({...p,[f.id]:e.target.value}))} style={inp}/>}</div>)}</div></div>)}</div></Side>;
}
...
function EditCell({value,onSave,type,options}){
  ...
  if(!editing)return<span onDoubleClick={()=>setEditing(true)} style={{cursor:"text"}}>{value}</span>;
  const done=()=>{setEditing(false);if(val!==value)onSave(val)};
  if(options)return<select ref={ref} value={val} onChange={e=>{setVal(e.target.value);onSave(e.target.value);setEditing(false)}} onBlur={()=>setEditing(false)} style={{...inp,width:"auto",fontSize:12,padding:"2px 6px"}}>{options.map(o=><option key={o} value={o}>{o}</option>)}</select>;
  if(type==="number")return<input ref={ref} type="number" value={val} onChange={e=>setVal(+e.target.value)} onBlur={done} onKeyDown={e=>e.key==="Enter"&&done()} style={{...inp,width:70,fontSize:12,padding:"2px 6px"}}/>;
  return<input ref={ref} value={val} onChange={e=>setVal(e.target.value)} onBlur={done} onKeyDown={e=>e.key==="Enter"&&done()} style={{...inp,width:140,fontSize:12,padding:"2px 6px"}}/>;
}

/* ═══ MAIN ═══ */
const COLS=[{k:"name",l:"Company",w:190},{k:"segment",l:"Segment",w:145},{k:"region",l:"Region",w:105},{k:"priority",l:"Prio",w:60},{k:"status",l:"Status",w:115},{k:"revenue",l:"Rev (m€)",w:80},{k:"employees",l:"Emp",w:70},{k:"sites",l:"Sites",w:50},{k:"outsourcingPropensity",l:"Outsrc",w:78}];

export default function App(){
  const[companies,setCo]=useState(ALL);
  ...
  const updateCo=useCallback((id,key,val)=>{setCo(prev=>prev.map(c=>c.id===id?{...c,[key]:val}:c))},[]);

  const filtered=useMemo(()=>{let d=[...companies];if(search){const q=search.toLowerCase();d=d.filter(c=>c.name.toLowerCase().includes(q)||c.nip.includes(q))}if(segF!=="All")d=d.filter(c=>c.segment===segF);if(staF!=="All")d=d.filter(c=>c.status===staF);if(priF!=="All")d=d.filter(c=>c.priority===priF);d.sort((a,b)=>{let av=a[sortK],bv=b[sortK];if(typeof av==="string"){av=av.toLowerCase();bv=bv.toLowerCase()}return sortD==="asc"?(av<bv?-1:av>bv?1:0):(av>bv?-1:av<bv?1:0)});return d},[companies,search,segF,staF,priF,sortK,sortD]);
  ...
  const segData=useMemo(()=>SEGMENTS.map(s=>({k:s.replace("& ",""),v:filtered.filter(c=>c.segment===s).length})),[filtered]);
  const staData=useMemo(()=>STATUSES.map(s=>({k:s,v:filtered.filter(c=>c.status===s).length})),[filtered]);
  const regData=useMemo(()=>REGIONS.map(r=>({k:r,v:filtered.filter(c=>c.region===r).length})).sort((a,b)=>b.v-a.v),[filtered]);
  const revData=useMemo(()=>SEGMENTS.map(s=>({k:s.split(" ")[0],v:Math.round(filtered.filter(c=>c.segment===s).reduce((a,c)=>a+c.revenue,0))})),[filtered]);
  ...
  return<div style={{fontFamily:F,background:"#f3f4f6",minHeight:"100vh",color:Dk}}>
    ...
    <header style={{background:Dk,padding:"12px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:32,height:32,borderRadius:8,background:P,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,color:"#fff",fontSize:14}}>A</div><div><div style={{fontSize:14,fontWeight:800,color:"#fff"}}>ATALIAN <span style={{fontWeight:400,opacity:.5}}>Poland</span></div></div></div>
      <div style={{display:"flex",gap:3}}>{[{id:"dashboard",lb:"Dashboard"},{id:"table",lb:"Hunt List"}].map(v=><button key={v.id} onClick={()=>{setView(v.id);setPage(0)}} style={{padding:"5px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:F,background:view===v.id?P:"transparent",color:view===v.id?"#fff":"rgba(255,255,255,.5)"}}>{v.lb}</button>)}</div>
    </header>
    ...
    <main style={{padding:"18px 24px 36px",maxWidth:1380,margin:"0 auto"}}>
      <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:18}}>
        ...
        <KPI label="P1 Priority" value={p1.toLocaleString()} sub={`${((p1/(filtered.length||1))*100).toFixed(0)}%`} accent={P}/>
        <KPI label="New Suspects" value={ns.toLocaleString()} sub="to contact" accent="#f59e0b"/>
        <KPI label="Avg Revenue" value={`${avg}m€`} sub="per company" accent="#16a34a"/>
      </div>

      {view==="dashboard"?<div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          <div style={{...cd,padding:"18px 20px"}}><h3 style={{margin:"0 0 8px",fontSize:14,fontWeight:800}}>Pipeline Funnel</h3><DynWaterfall data={filtered}/></div>
          <div style={{...cd,padding:"18px 20px"}}><h3 style={{margin:"0 0 8px",fontSize:14,fontWeight:800}}>By Segment</h3><BarChart data={segData} colorMap={Object.fromEntries(SEGMENTS.map(s=>[s.replace("& ",""),SEGC[s]]))}/></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
          <div style={{...cd,padding:"18px 20px"}}><h3 style={{margin:"0 0 12px",fontSize:14,fontWeight:800}}>By Status</h3><div style={{display:"flex",alignItems:"center",gap:16}}><DonutChart data={staData} colorMap={STAC} size={140}/><div style={{display:"flex",flexDirection:"column",gap:6}}>{staData.map(d=><div key={d.k} style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:10,height:10,borderRadius:3,background:STAC[d.k]}}/><span style={{fontSize:11,color:"#6b7280"}}>{d.k}</span><span style={{fontSize:11,fontWeight:800,color:Dk,marginLeft:"auto"}}>{d.v}</span></div>)}</div></div></div>
          <div style={{...cd,padding:"18px 20px"}}><h3 style={{margin:"0 0 8px",fontSize:14,fontWeight:800}}>Revenue by Segment</h3><BarChart data={revData} colorMap={Object.fromEntries(SEGMENTS.map(s=>[s.split(" ")[0],SEGC[s]]))}/></div>
          <div style={{...cd,padding:"18px 20px"}}><h3 style={{margin:"0 0 8px",fontSize:14,fontWeight:800}}>By Region</h3><BarChart data={regData} colorMap={REGC}/></div>
        </div>
      </div>:

      <div>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12,alignItems:"center"}}>
          <input placeholder="Search…" value={search} onChange={e=>{setSearch(e.target.value);setPage(0)}} style={{...inp,width:200,background:"#fff"}}/>
          {[{v:segF,s:setSegF,o:["All",...SEGMENTS],l:"Segment"},{v:staF,s:setStaF,o:["All",...STATUSES],l:"Status"},{v:priF,s:setPriF,o:["All",...PRIORITIES],l:"Priority"}].map(({v,s,o,l})=><select key={l} value={v} onChange={e=>{s(e.target.value);setPage(0)}} style={{...inp,width:"auto",background:"#fff",cursor:"pointer"}}>{o.map(oo=><option key={oo} value={oo}>{oo==="All"?`${l}`:oo}</option>)}</select>)}
          <span style={{fontSize:10,color:"#9ca3af",marginLeft:"auto",fontWeight:600}}>{filtered.length} results · double-click to edit</span>
        </div>
        <div style={{...cd,overflow:"hidden"}}><div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr>{COLS.map(c=><th key={c.k} onClick={()=>hs(c.k)} style={{padding:"9px 12px",textAlign:"left",fontSize:9,fontWeight:800,textTransform:"uppercase",letterSpacing:".08em",color:sortK===c.k?P:"#9ca3af",borderBottom:"2px solid #f0f0f5",cursor:"pointer",userSelect:"none",whiteSpace:"nowrap",minWidth:c.w,background:"#fafafa"}}>{c.l}{sortK===c.k?(sortD==="asc"?" ↑":" ↓"):""}</th>)}</tr></thead>
            <tbody>{pg.map((c,i)=>{const q=store.qual(c.id);const hf=store.hasFile(c.id);const isH=hovRow===c.id;
              return<tr key={c.id} style={{background:isH?"#eef2ff":i%2===0?"#fff":"#fafafa",transition:"background .1s",position:"relative"}} onMouseEnter={()=>setHovRow(c.id)} onMouseLeave={()=>setHovRow(null)}>
                {COLS.map(col=><td key={col.k} style={{padding:"8px 12px",borderBottom:"1px solid #f0f0f5",whiteSpace:"nowrap",color:"#374151"}}>
                  {col.k==="name"?<div style={{display:"flex",alignItems:"center"}}><EditCell value={c.name} onSave={v=>updateCo(c.id,"name",v)}/>{q&&<QBadge q={q}/>}{hf&&<span style={{marginLeft:3,fontSize:9,color:P}}>📋</span>}</div>:
                  col.k==="segment"?<EditCell value={c.segment} onSave={v=>updateCo(c.id,"segment",v)} options={SEGMENTS}/>:
                  col.k==="region"?<EditCell value={c.region} onSave={v=>updateCo(c.id,"region",v)} options={REGIONS}/>:
                  col.k==="priority"?<EditCell value={c.priority} onSave={v=>updateCo(c.id,"priority",v)} options={PRIORITIES}/>:
                  col.k==="status"?<EditCell value={c.status} onSave={v=>updateCo(c.id,"status",v)} options={STATUSES}/>:
                  col.k==="revenue"?<EditCell value={c.revenue} onSave={v=>updateCo(c.id,"revenue",+v)} type="number"/>:
                  col.k==="employees"?<EditCell value={c.employees} onSave={v=>updateCo(c.id,"employees",+v)} type="number"/>:
                  col.k==="sites"?<EditCell value={c.sites} onSave={v=>updateCo(c.id,"sites",+v)} type="number"/>:
                  col.k==="outsourcingPropensity"?<EditCell value={c.outsourcingPropensity} onSave={v=>updateCo(c.id,"outsourcingPropensity",v)} options={["High","Medium","Low"]}/>:
                  c[col.k]}
                </td>)}
                {isH&&<td style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",display:"flex",gap:4,zIndex:5}}>
                  <button onClick={()=>setPanel({type:"prequal",co:c})} style={{padding:"5px 12px",borderRadius:8,border:"none",background:P,color:"#fff",fontSize:10,fontWeight:800,cursor:"pointer",fontFamily:F,boxShadow:`0 2px 10px ${P}44`}}>🎯 Pre-qual</button>
                  <button onClick={()=>setPanel({type:"prospect",co:c})} style={{padding:"5px 12px",borderRadius:8,border:"none",background:Dk,color:"#fff",fontSize:10,fontWeight:800,cursor:"pointer",fontFamily:F,boxShadow:`0 2px 10px ${Dk}44`}}>📋 Prospect</button>
                </td>}
              </tr>})}</tbody>
          </table></div></div>
        <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:6,marginTop:12}}>
          <button disabled={page===0} onClick={()=>setPage(page-1)} style={{padding:"5px 14px",borderRadius:8,border:"1px solid #e5e7eb",background:page===0?"#f3f4f6":"#fff",cursor:page===0?"default":"pointer",fontSize:11,fontFamily:F,color:page===0?"#d1d5db":Dk,fontWeight:700}}>←</button>
          <span style={{fontSize:11,color:"#6b7280",fontWeight:700}}>{page+1}/{tp}</span>
          <button disabled={page>=tp-1} onClick={()=>setPage(page+1)} style={{padding:"5px 14px",borderRadius:8,border:"1px solid #e5e7eb",background:page>=tp-1?"#f3f4f6":"#fff",cursor:page>=tp-1?"default":"pointer",fontSize:11,fontFamily:F,color:page>=tp-1?"#d1d5db":Dk,fontWeight:700}}>→</button>
        </div>
      </div>}
    </main>
