...
function ODot(props){var v=props.v;var c=v>=5?"#16a34a":"#f59e0b";return<span style={{display:"flex",alignItems:"center",gap:5,fontSize:11}}><span style={{width:8,height:8,borderRadius:"50%",background:c}}/>{v}</span>}
...
function ScoreBtn(props){return<div style={{display:"flex",gap:4}}>{[1,2,3,4,5].map(function(n){return<button key={n} onClick={function(){props.onChange(n)}} style={{width:36,height:36,borderRadius:10,border:props.value===n?"2.5px solid "+props.color:"2px solid #e5e7eb",background:props.value===n?props.color:"#fff",color:props.value===n?"#fff":"#9ca3af",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{n}</button>})}</div>}
...
export default function App(){
  ...
  var ccc=filtered.filter(function(c){return c.cluster==="Current Client"}).length;
  var privc=filtered.filter(function(c){return c.ownership==="Private"}).length;
  var pubc=filtered.length-privc;
  ...
  return<div style={{fontFamily:F,background:"#f3f4f6",minHeight:"100vh",color:Dk}}>
    ...
    <main style={{padding:"18px 24px 36px",maxWidth:1400,margin:"0 auto"}}>
      <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:18}}>
        <KPI label="Companies" value={filtered.length} sub="in scope" accent={Dk}/>
        <KPI label="New Prospects" value={npc} sub={(npc/(filtered.length||1)*100).toFixed(0)+"%" } accent={P}/>
        <KPI label="Avg Revenue" value={avgRev+"bn€"} sub="per company" accent="#16a34a"/>
        <KPI label="Avg Score" value={avgScore+"/5"} sub="prioritization" accent="#2563eb"/>
      </div>
      ...
      <div>
        ...
        <div style={Object.assign({},cd,{padding:"10px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"})}>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{COLS.map(function(col){var on=visibleCols.includes(col.k);return<button key={col.k} onClick={function(){setVisibleCols(function(prev){return on?prev.filter(function(k){return k!==col.k}):prev.concat([col.k])})}} style={{padding:"3px 10px",borderRadius:20,border:on?"2px solid "+P:"1.5px solid #e5e7eb",background:on?P+"15":"#fff",color:on?P:"#9ca3af",fontSize:10,fontWeight:on?700:500,cursor:"pointer",fontFamily:F,transition:"all .15s"}}>{col.l}</button>})}</div>
          <div style={{marginLeft:"auto",display:"flex",gap:4}}><button onClick={function(){setVisibleCols(COLS.map(function(c){return c.k}))}} style={{padding:"3px 10px",borderRadius:20,border:"1.5px solid #e5e7eb",background:"#fff",color:"#6b7280",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:F}}>All</button><button onClick={function(){setVisibleCols(["name","segment","ownership","revenueBnEur","cluster","score","contactName"])}} style={{padding:"3px 10px",borderRadius:20,border:"1.5px solid #e5e7eb",background:"#fff",color:"#6b7280",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:F}}>Minimal</button></div>
        </div>
